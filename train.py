...
Trains a calibrated XGBClassifier on historical match data and saves:
  ...
  - models/feature_names.pkl   : ordered feature column names
...
def load_and_prepare(path: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(path, parse_dates=["MatchDate"], low_memory=False)
...
def build_elo_state(df: pd.DataFrame) -> dict:
    """Return the most-recent Elo per team, scraped from match records."""
    df_sorted = df.sort_values("MatchDate")
    elo_state: dict = {}
    for _, row in df_sorted.iterrows():
        if not pd.isna(row.get("HomeElo", float("nan"))):
            elo_state[row["HomeTeam"]] = float(row["HomeElo"])
        if not pd.isna(row.get("AwayElo", float("nan"))):
...
def main():
    ...
    with open(MODELS_DIR / "xgb_model.pkl",    "wb") as f: pickle.dump(model,     f)
    with open(MODELS_DIR / "elo_state.pkl",    "wb") as f: pickle.dump(elo_state, f)
    with open(MODELS_DIR / "team_list.pkl",    "wb") as f: pickle.dump(team_list, f)
    with open(MODELS_DIR / "feature_names.pkl","wb") as f: pickle.dump(FEATURES,  f)
